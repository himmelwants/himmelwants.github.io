...
# 0. 지난 주차 수업 내용 복습
    ...
    #1. 도서번호가 5인 도서의 도서명, 출판사, 가격을 출력하라
    selct bookname, publisher, price
    from Book
    where bookid = 5;

이미지

    #2. 가격이 20000원 이상인 도서정보 출력하라
    select *
    from Book
    where price>=20000;
...
이미지
    ...
    #3. 도서 가격이 15000원 이상 20000원 이하인 도서 정보를 도서명 내림차순으로 정렬해 출력하라
    select *
    from Book
...
이미지
    ...
    #4. 보유하고 있는 도서의 총 개수를 출력하라
    select count(bookid)
    from Book;

이미지
  
    #5. 거래하고 있는 출판사수를 출력하라
    select count(distinct publisher)
    from Book;

이미지

    #6. 도서를 한번이라도 구매한 적이 있는 고객 수를 구하여라
    select count(distinct custid)
    from Orders;

이미지
  
    #7. 성이 'kim'인 고객 정보를 출력하라
    select *
    from Customer
    where name like 'kim%';
...
이미지
    ...
    #8. 성이 'kim'이고 이름이 'a'로 끝나는 고객 정보 출력하라
    select *
    from Customer
...
이미지
    ...
    #9. 핸드폰 번호가 null 값이 들어가 있는 고객 정보를 출력하라
    select *
    from Customer
    where phone is null;
...
이미지
    ...
    #10. 고객번호가 2인 고객의 도서 총 구매액을 구하여라 
...
insert into 데이터 추가 
    ...
    insert  into NewBook values(2,'book2','tukorea',150000);
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE3OTAwNzQzMTcsLTIxMTUyMTA1MTksLT
cxNTYwNTA5MCwtNzczNzEyMDkyLDI3MTk1MTY5Nyw2ODYxNzgw
ODksLTQwMTg5MzAwMywyNzQ5ODc0NTMsLTE1MzI3ODMzNzgsNj
k1MzQ0Mjc3LC0xMDE5OTA2Nzc2LC0yMjY3NTE1MjRdfQ==
-->