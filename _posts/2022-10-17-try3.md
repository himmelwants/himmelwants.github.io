...
# 0. 지난 주차 수업 내용 복습
    ...
    #1. 도서번호가 5인 도서의 도서명, 출판사, 가격을 출력
    selct bookname, publisher, price
    from Book
    where bookid = 5;

    #2. 가격이 20000원 이상인 도서정보 출력
    select *
    from Book
    where price>=20000;
...
#8. 성이 'kim'이고 이름이 'a'로 끝나는 고객 정보 출력 
select *
from Customer
where name like 'kim%a';

#9. 핸드폰 번호가 null 값이 들어가 있는 고객 정보 
select *
from Customer
where phone is null;
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE0OTQyODgyMSwtMjExNTIxMDUxOSwtNz
